...
const Register = (props) => {
  const register = (values, { setSubmitting, resetForm}) => {
    try{
      auth().createUserWithEmailAndPassword(values.name ,values.email, values.password).then(() => {
        resetForm({});
      }).catch((error) => {
          ...
          <View style={style.formContainer}>
            ...
            <View>
              <TouchableOpacity onPress={() => props.navigation.navigate('Login')}>
                <Text style={{textAlign:'center', fontSize:18, color:'#fff'}}>Hesabınız var ise giriş yapın</Text>
              </TouchableOpacity>
            </View>
          </View>
        )}
